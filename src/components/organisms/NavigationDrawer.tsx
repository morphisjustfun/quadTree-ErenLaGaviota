...
export type LabelActive = "QUADTREE" | "ABOUT";
...
const NavigationDrawer: FC<INavigationDrawerProps> = (props) => {
  ...
  return (
    <div className="relative min-h-screen flex">
      <div
        className={`fixed h-full bg-gradient-to-t from-light-surface-gradient to-light-surface-gradient bg-light-surface1 text-light-primary py-4 transition-width duration-500 ${
          open.current ? "2xl:w-96 md:w-48 w-24 px-3" : "w-0"
        }`}
        ref={containerRef}
      >
        <div className={`${open.current ? "" : "hidden"}`} ref={navRef}>
          ...
          <span className="py-4 text-light-primary font-bold px-6 tracking-wide text-xl hidden md:block">
            TEAM EREN LA GAVIOTA
          </span>
          <nav>
            <Label
              icon={AboutMeIcon}
              active={props.labelActive === "QUADTREE"}
              href="/"
            >
              Quadtree
            </Label>
            <Label
              icon={WorkIcon}
              active={props.labelActive === "ABOUT"}
              href="/about"
            >
              About
            </Label>
